...
void WrCmdDat (U32 cmd, U32 val) {
  ...
  LPC_USB->USBDevIntClr = CCEMTY_INT;									// 清零USB设备中断状态寄存器的CCEMPTY位，表示有命令正在写入
  LPC_USB->USBCmdCode = cmd;													// 写入命令
  while ((LPC_USB->USBDevIntSt & CCEMTY_INT) == 0);		// 命令执行完毕，CCEMPTY位置位
  LPC_USB->USBDevIntClr = CCEMTY_INT;									// 再次清零USB设备中断状态寄存器的CCEMPTY位，表示有命令正在写入
  LPC_USB->USBCmdCode = val;													// 写入参数
  while ((LPC_USB->USBDevIntSt & CCEMTY_INT) == 0);		
}
...
void USB_Reset (void) {
  ...
  LPC_USB->USBEpInd = 0;																				//USBEpInd端点索引寄存器，指向端点0，端点0是控制输出端点
  LPC_USB->USBMaxPSize = USB_MAX_PACKET0;												//设置端点0的最大包长度
  LPC_USB->USBEpInd = 1;																				//指向端点1，端点1是控制输入端点
  LPC_USB->USBMaxPSize = USB_MAX_PACKET0;												//设置端点1的最大包长度，
  while ((LPC_USB->USBDevIntSt & EP_RLZED_INT) == 0);						//USBDevIntSt是USB设备中断状态寄存器，等待EP_RELZED置位，表示端点被使用

  LPC_USB->USBEpIntClr = 0xFFFFFFFF;														//清除所有端点的中断状态
  LPC_USB->USBEpIntEn   = 0xFFFFFFFF ^ USB_DMA_EP;							//使能端点的中断
  LPC_USB->USBDevIntClr = 0xFFFFFFFF;														//清除设备中断
